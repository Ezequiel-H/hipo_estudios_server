...
export const getModelFields = model => Object.keys(model.schema.paths).map(field => field.split('.')[0]);
...
const mapMongoQuery = (query, field) => {
  const object = {};
  if (typeof query !== 'object' || field === 'any') {
    return query;
  }
...
export const buildQuery = (Model, query, initialFilter, initialSort, initialLimit) => {
  const name = Model.collection.collectionName;
  if (!query[name]) {
    return Model.find(initialFilter, null, { sort: initialSort, limit: initialLimit });
  }
  const filterQuery = query[name].filter ? { ...query[name].filter, ...initialFilter } : initialFilter;
  ...
  const sort = query[name].sort ? { ...query[name].sort, ...initialSort } : initialSort;
  const limit = query[name].limit ? parseInt(query[name].limit, 10) : initialLimit;
